...
export default function AdminProgramCoursesPage() {
    ...
    const getStatusBadge = (status: string | null) => {
        ...
        switch (status.toLowerCase()) {
            case 'active': return <Badge variant="default" className="bg-gradient-to-r from-green-500 to-emerald-600 text-white border-0">Active</Badge>;
                                                ...
                                                {Object.keys(programData[branch]).sort((a, b) => parseInt(a) - parseInt(b)).map(semesterStr => {
                                                    ...
                                                    return (
                                                        <div key={`${programName}-${branch}-${semester}`} className="mb-4">
                                                            ...
                                                            <Table><TableHeader><TableRow className="border-border/30"><TableHead>ID</TableHead><TableHead>Name</TableHead><TableHead className="text-center">Credits</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                                                                <TableBody>{programData[branch][semester].map(course => (
                                                                    <TableRow key={course.id} className="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 border-border/20"><TableCell className="font-medium">{course.courseId}</TableCell><TableCell>{course.courseName}</TableCell><TableCell className="text-center">{course.credits}</TableCell>
