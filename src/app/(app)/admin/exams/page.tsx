...
export default function AdminExamSchedulePage() {
  ...
  const handleOpenAddDialog = () => {
    setDialogState({ isOpen: true, mode: 'add', initialData: null });
  };

  const handleOpenEditDialog = (schedule: ExamSchedule) => {
    setDialogState({ isOpen: true, mode: 'edit', initialData: schedule });
  };
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-orange-600 to-red-500 p-6 mb-6 text-white shadow-lg">
        <div className="relative z-10 flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-white/70 text-sm mt-1">Schedule, edit, and manage examination timetables</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={handleOpenAddDialog} className="gap-2 bg-white text-orange-700 hover:bg-white/90 shadow-md"><PlusCircle className="h-4 w-4" /> Schedule Exams</Button>
          </div>
        </div>
      ...
      <Card className="card-elevated">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Accordion type="single" collapsible className="w-full" defaultValue={Object.keys(groupedSchedules)[0]}>
              {Object.entries(groupedSchedules).map(([sessionName, schedules]) => (
                <AccordionItem key={sessionName} value={sessionName}>
                  <AccordionTrigger className="text-lg font-medium">{sessionName} <Badge className="ml-2 bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 border-0">{schedules.length} exams</Badge></AccordionTrigger>
                  <AccordionContent>
                    <Table>
                      <TableHeader>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3"><div className="rounded-2xl bg-muted p-3"><Ticket className="h-6 w-6 text-muted-foreground" /></div><p className="text-sm text-muted-foreground">No exam schedules found.</p></div>
          )}
        </CardContent>
      </Card>
