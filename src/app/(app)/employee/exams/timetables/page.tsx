...
import { Loader2, User, Users, Search as SearchIcon, AlertTriangle, Edit } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { TimetableGrid } from '@/components/shared/TimetableGrid';
import type { ScheduleEntry } from '@/lib/types';
import { getTimetableFilters, getScheduleForClass, saveTimetableBulk, type FullSchedule, type TimetableFilters } from '@/actions/timetable-actions';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";


export default function EmployeeTimetablesPage() {
        ...
        <div className="space-y-8 animate-fade-in">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-teal-600 to-emerald-500 p-6 mb-6 text-white shadow-lg">
                <div className="relative z-10">
                    ...
                    <p className="text-white/70 text-sm mt-1">Create, edit, and view timetables for class sections — with student and teacher views</p>
                </div>
            ...
            <Card className="card-elevated">
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                        <div>
                            <Label>Program</Label>
                            <Select value={selected.program} onValueChange={(v) => handleFilterChange('program', v)} disabled={isLoadingFilters}>
                                <SelectTrigger><SelectValue placeholder="Select Program" /></SelectTrigger>
                                <SelectContent>{filters.programs.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
                        <div>
                            <Label>Branch</Label>
                            <Select value={selected.branch} onValueChange={(v) => handleFilterChange('branch', v)} disabled={!selected.program}>
                                <SelectTrigger><SelectValue placeholder="Select Branch" /></SelectTrigger>
                                <SelectContent>{availableBranches.map(b => <SelectItem key={b} value={b}>{b}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
                        <div>
                            <Label>Semester</Label>
                            <Select value={selected.semester} onValueChange={(v) => handleFilterChange('semester', v)} disabled={isLoadingFilters}>
                                <SelectTrigger><SelectValue placeholder="Select Semester" /></SelectTrigger>
                                <SelectContent>{filters.semesters.map(s => <SelectItem key={s} value={String(s)}>Sem {s}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
                        <div>
                            <Label>Section</Label>
                            <Select value={selected.section} onValueChange={(v) => handleFilterChange('section', v)} disabled={!selected.program}>
                                <SelectTrigger><SelectValue placeholder="Select Section" /></SelectTrigger>
                                <SelectContent>{availableSections.map(s => <SelectItem key={s} value={s}>Sec {s}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
                    </div>
                ...
                <Tabs value={mode} onValueChange={(v) => setMode(v as any)} className="w-full">
                    ...
                    <TabsContent value="edit">
                        <div className="flex justify-between items-center mb-4 mt-4">
                            ...
                            <div className="flex gap-2">
                                {hasUnsavedChanges && (
                                    <Button size="sm" onClick={async () => {
                                        const slots = draftSchedule.map(s => {
                                            ...
                                            return { day: s.day, period: s.period, courseId: s.courseCode, teacherId };
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleFetchSchedule()}>Discard Changes</Button>
                            </div>
                        </div>
                        <TimetableGrid
                    ...
                    <TabsContent value="student">
                        <div className="flex justify-between items-center mb-4 mt-4">
                            ...
                            <Button variant="outline" size="sm" onClick={() => handleFetchSchedule()}>Refresh</Button>
                        </div>
