            ...
            <div className="rounded-2xl bg-background/95 backdrop-blur-xl dark:bg-background/80">
                <div className="flex items-center px-4 sm:px-5 py-3.5 gap-3">
                    ...
                    <button
                        ...
                        className="flex items-center justify-center h-8 w-8 rounded-xl hover:bg-violet-500/10 border border-violet-500/20 text-violet-500 transition-colors shrink-0"
                    >
...
export function ClassroomDetailPage({ basePath }: { basePath: string }) {
    ...
    React.useEffect(() => {
        async function loadUser() {
            ...
            if (profile) {
                setCurrentUser({ uid: profile.uid, name: profile.name, role: profile.role } as any);
            } else {
