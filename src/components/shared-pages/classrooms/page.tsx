...
function CreateClassDialog({ isOpen, onOpenChange, onClassroomCreated, currentUser }: { isOpen: boolean; onOpenChange: (open: boolean) => void; onClassroomCreated: () => void; currentUser: AppUser | null }) {
    ...
    const handleCreate = async () => {
        ...
        if (!currentUser) {
            toast({ variant: 'destructive', title: 'Error', description: 'Could not identify the user. Please refresh and try again.' });
            return;
        }
        setIsLoading(true);
        const result = await createClassroom({ name, section, subject, ownerId: currentUser.uid, ownerName: currentUser.name || currentUser.email || 'Teacher' });
...
function JoinClassDialog({ isOpen, onOpenChange, onClassroomJoined, currentUser }: { isOpen: boolean; onOpenChange: (open: boolean) => void; onClassroomJoined: () => void; currentUser: AppUser | null }) {
    const { toast } = useToast();
    ...
    const handleJoin = async () => {
        ...
        setIsLoading(true);
        const result = await joinClassroom({ inviteCode: inviteCode.trim().toUpperCase(), studentUid: currentUser.uid });
        if (result.success) {
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent>
                ...
                <div className="py-4">
                    ...
                    <Input id="code" value={inviteCode} onChange={e => setInviteCode(e.target.value.toUpperCase())} placeholder="e.g., ABC123" className="mt-1 font-mono tracking-widest text-center text-lg" />
                </div>
                <DialogFooter>
                    <DialogClose asChild><Button variant="outline" disabled={isLoading}>Cancel</Button></DialogClose>
                    <Button onClick={handleJoin} disabled={isLoading}>{isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Joining...</> : 'Join Class'}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    );
...
export function ClassroomsPage({ basePath, mode = 'auto' }: { basePath: string; mode?: 'teacher' | 'student' | 'auto' }) {
    const { toast } = useToast();
    ...
    const loadData = React.useCallback(async () => {
        ...
        try {
            ...
            const user = { uid: profile.uid, name: profile.name, role: profile.role } as AppUser;
            setCurrentUser(user);
            const data = await getMyClassrooms(profile.uid);
            setClassrooms(data);
        } catch (error: any) {
    ...
    const isTeacher = mode === 'teacher' || (mode === 'auto' && currentUser?.role === 'teacher');
    ...
    return (
        <>
                        ...
                        <Card key={room.id} className="flex flex-col relative group card-elevated">
                            ...
                            <CardFooter>
                                <Button className="w-full" asChild>
                                    <Link href={`${basePath}/${room.id}`}><BookOpen className="mr-2 h-4 w-4"/>Open</Link>
                                </Button>
                            </CardFooter>
                        </Card>
                    ))}
            ...
            {isTeacher ? (
                 <CreateClassDialog isOpen={isCreateOpen} onOpenChange={setIsCreateOpen} onClassroomCreated={loadData} currentUser={currentUser} />
            ) : (
                 <JoinClassDialog isOpen={isJoinOpen} onOpenChange={setIsJoinOpen} onClassroomJoined={loadData} currentUser={currentUser} />
            )}
